from api.serializers import InstallPayloadSerializer
...
class InstallPingAPIView(APIView):
  ...
  permission_classes = []
  def get(self, request):
    installpayload_obj = InstallPayload.objects
    label = "Total Installaion"
    w_label = max(50, 8 * len(label) + 20)
    w_value = max(30, 14 + len(str(installpayload_obj.count())) * 8)
    w_total = w_label + w_value
    h = 20
    svg = f'''
    <svg xmlns="http://www.w3.org/2000/svg" width="{w_total}" height="{h}">
      ...
      <g fill="#fff" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11">
        <text x="{w_label/2}" y="14" text-anchor="middle">{label}</text>
        <text x="{w_label + w_value/2}" y="14" text-anchor="middle">{installpayload_obj.count()}</text>
      </g>
    </svg>
    '''
    visitors_obj.create()
